...
const MarketPrices = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const q = query(collection(db, "market_prices"), orderBy("timestamp", "desc"), limit(100));
                ...
                <div key={item.id || idx} className="bg-white rounded-3xl border border-gray-100 p-5 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col group animate-fade-up">
                  ...
                  <div className="flex justify-between items-start mb-4">
                     ...
                     <div className="shrink-0 flex items-center gap-2">
                       <ShareButton title={`🌾 Mandi Price: ${item.crop}`} text={`The latest price at ${item.market}, ${item.district} is *${item.price}*.`} className="w-7 h-7 bg-white text-green-500 hover:bg-green-50 rounded-full border border-gray-100 shadow-sm" />
                       {item.source === 'govt' ? (
                   ...
                   <div className="space-y-6">
                      ...
                      <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
                        <div className="flex justify-between items-center mb-4 px-2">
                            <h4 className="text-sm font-bold text-slate-700 flex items-center gap-2">Price Trend Projection <TTSButton text={`The AI predicts the following trend for ${selectedCrop.crop} at ${selectedCrop.market}. ` + forecastData.map(w => `${w.week}: ${w.price} rupees per quintal. Signal: ${w.signal}. ${w.reason}`).join(". ")} /></h4>
