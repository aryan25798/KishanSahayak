...
const EquipmentMarketplace = ({ adminOverride = false }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      // 1. Fetch Equipment
      const itemsSnap = await getDocs(query(collection(db, "equipment"), orderBy("createdAt", "desc"), limit(100)));
  ...
  const handlePostItem = async (e) => {
    ...
    try {
      ...
      if (image) {
          ...
          const imgRef = ref(storage, `equipment/${Date.now()}_${compressedImage.name}`);
                    ...
                    <div className="absolute bottom-5 left-5 right-5 text-white">
                      <div className="flex justify-between items-end mb-1">
                          ...
                          <div className="flex items-center gap-2">
                             <ShareButton title={`🚜 Equipment: ${item.name}`} text={`${item.description}. Price: ₹${item.price}. Call/Book via Kisan Sahayak!`} className="w-8 h-8 rounded-full p-0 flex items-center justify-center bg-white/20 text-white backdrop-blur-md border border-white/30 hover:bg-green-500 hover:border-green-400 transition" />
                             {rating && (
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    <div className="flex justify-between items-end mb-5 pb-5 border-b border-gray-50">
                      ...
                      <div className="flex flex-col items-end">
                          ...
                          <span className="text-sm font-bold text-slate-800 flex items-center gap-1 bg-gray-50 px-2 py-1 rounded-lg" title={rating && rating.avg >= 4.0 && rating.count > 0 ? "Highly Rated Trusted Owner" : ""}>
                             <User size={12} className="text-orange-500"/> {item.ownerName}
                             {rating && rating.avg >= 4.0 && rating.count > 0 && <ShieldCheck size={14} className="text-blue-500 ml-1" />}
                          </span>
                      </div>
                    </div>
