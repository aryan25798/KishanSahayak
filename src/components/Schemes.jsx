  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const schemesSnap = await getDocs(query(schemesRef, limit(100))); 
        ...
        if (user) {
          ...
          const q = query(appsRef, where("uid", "==", user.uid), limit(100));
                                    ...
                                    <div className="p-6 flex flex-col flex-1">
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <div className="flex gap-2">
                                                <TTSButton text={`${scheme.title}. ${scheme.description}`} className="w-8 h-8 rounded-full p-0 flex items-center justify-center" />
                                                <ShareButton title={`📜 Scheme: ${scheme.title}`} text={scheme.description} className="w-8 h-8 rounded-full p-0 border-none bg-green-50 flex items-center justify-center text-green-600" />
                                            </div>
                                        </div>
