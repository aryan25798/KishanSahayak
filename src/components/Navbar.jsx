...
const Navbar = () => {
  ...
  // ✅ OPTIMIZATION: Track 'isScrolled' separately to reduce re-renders. 
  // We no longer store every pixel of 'lastScrollY' in state.
  ...
  // ⚠️ SECURITY NOTE: This check is for UI visibility only. 
  // Real data security MUST be enforced in Firestore Security Rules.
    ...
    const handleScroll = () => {
      if (!ticking) {
        window.requestAnimationFrame(() => {
          ...
          // 1. Toggle Glassmorphism Background (State only updates when crossing threshold)
          ...
          // 2. Hide/Show Navbar based on direction
          // Hide on scroll down (>50px), show on scroll up
          if (currentScrollY > previousScrollY && currentScrollY > 50 && !isMobileMenuOpen) {
            setIsVisible(false);
          } else {
            setIsVisible(true);
          }
  ...
  const handleVoiceNav = () => {
    ...
    try {
      ...
      recognition.onstart = () => {
        ...
        // ✅ LOGIC PRESERVED: Auto-stop silence timer
