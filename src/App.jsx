...
// ✅ NEW: Import the Reset Password Page
const ResetPassword = lazy(() => import("./components/ResetPassword"));

// ✅ NEW: Error Boundary to catch "ChunkLoadError" on spotty networks
class ErrorBoundary extends React.Component {
  ...
  static getDerivedStateFromNewError(error) {
      ...
      return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] p-6 text-center">
          ...
          <p className="text-slate-600 mb-6 max-w-md">
            We couldn't load this section. This usually happens when the network connection is weak.
          </p>
          <button 
...
// ✅ IMPROVED: Smart Loading Spinner with Timeout Message
const PageLoader = () => {
  const [showSlowMessage, setShowSlowMessage] = useState(false);

  useEffect(() => {
    // If loading takes more than 5 seconds, show a reassuring message
  ...
  return (
    <div className="flex h-[80vh] w-full items-center justify-center bg-transparent px-4">
      <div className="flex flex-col items-center gap-4 p-8 bg-white/40 backdrop-blur-xl rounded-3xl border border-white/50 shadow-2xl">
        <Loader className="h-12 w-12 animate-spin text-emerald-600" />
        <div className="text-center">
          <span className="block font-bold text-lg text-emerald-900 animate-pulse">Loading...</span>
          {showSlowMessage && (
            <span className="block text-sm text-emerald-700 mt-2 font-medium bg-emerald-100/50 px-3 py-1 rounded-full animate-fade-in">
    ...
    <AuthProvider>
      <Router>
        ...
        <div className="min-h-screen font-sans text-slate-900 relative bg-slate-50 overflow-x-hidden">
          ...
          {/* Animated Gradient Orbs - Optimized for Mobile Performance */}
          <div className="fixed inset-0 z-0 pointer-events-none transform-gpu">
             {/* Base Gradient */}
             <div className="absolute inset-0 bg-gradient-to-br from-slate-50 via-emerald-50/50 to-blue-50/50"></div>
             
             {/* ✅ FIXED: Added 'will-change-transform' and 'transform-gpu' to force hardware acceleration */}
             <div className="absolute top-[-10%] left-[-10%] w-[80vw] sm:w-[50vw] h-[80vw] sm:h-[50vw] bg-emerald-300/20 rounded-full blur-[60px] sm:blur-[100px] animate-[pulse_10s_infinite_ease-in-out] will-change-transform transform-gpu"></div>
             <div className="absolute top-[40%] right-[-10%] w-[60vw] sm:w-[40vw] h-[60vw] sm:h-[40vw] bg-blue-300/20 rounded-full blur-[60px] sm:blur-[100px] animate-[pulse_15s_infinite_ease-in-out_delay-2000ms] will-change-transform transform-gpu"></div>
             <div className="absolute bottom-[-10%] left-[20%] w-[70vw] sm:w-[60vw] h-[70vw] sm:h-[60vw] bg-teal-200/20 rounded-full blur-[80px] sm:blur-[120px] animate-[pulse_20s_infinite_ease-in-out_delay-500ms] will-change-transform transform-gpu"></div>
          ...
          <div className="relative z-10 flex flex-col min-h-screen">
            ...
            <div className="flex-grow">
              {/* ✅ FIXED: Wrapped in ErrorBoundary to prevent white screens on network failure */}
              <ErrorBoundary>
